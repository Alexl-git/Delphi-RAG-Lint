...
| `glyph-vacuum --root D [--root D ...] --out D [--append] [--db X ...]` | measure every streamed graphic under the roots BEFORE writing a glyph rule: `instances.tsv` (one row per component+graphic property: wrapper/format/size/bpp, count property + value + declared default, inferred N, agree, sha), `classes.tsv` (per class: distributions, disagreements, runtime_refs), `skipped.tsv`, `images\`, `gallery.html`; `--append` merges; `--db` only qualifies |
