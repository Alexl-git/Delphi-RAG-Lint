...
| `glyph-vacuum --root D [--root D ...] --out\|--output D [--append] [--db X ...]` | measure every streamed graphic under the roots BEFORE writing a glyph rule: `instances.tsv` (one row per component+graphic property: wrapper/format/size/bpp, count property + value + declared default, inferred N, agree, sha), `classes.tsv` (per class: distributions, disagreements, runtime_refs), `skipped.tsv`, `images\`, `gallery.html`; `--append` merges; `--db` only qualifies |
...
| `index <path> --resolve-only` | re-derive call edges / ancestry / helper targets and purity verdicts from the STORED parses, skipping the walk. Use when `schema_meta.resolver_fingerprint` shows the edges predate the current resolver -- minutes, against the hours a re-parse costs, because no parse became wrong |
