...
| `query find-callers --name X` | callers of a symbol (`--context N`; `--resolved` for precise call-edge callers). **Every `--resolved` row names the CALL SITE** (since 2026-09-23): text prints `(<file>:<site line>)`, JSON `line` is the same number, and the caller routine's declaration line is the separate key `caller_line` (omitted at unit level); JSON key order `caller_qname, file, confidence, target_qname, line, caller_line, mode`. Before that, JSON `line` was the caller's DECLARATION line on every row except a callback. `--resolved` also reports routines **reached as a callback** -- handed somewhere by bare name, `@X`, or an event assignment -- marked `[callback]` rather than `[certain]`/`[ambiguous]`, because that is a reach, not a call. Without it a live predicate passed to e.g. `TDirectory.GetFiles` read as dead. **A PROPERTY or FIELD name works too (2026-09-16):** each bound access is a row tagged `[certain, read]` / `[certain, write]` (JSON `mode`) -- since resolver 1.8.0-alpha BARE reads count too: a member of a `with` target, a property of the enclosing class, `Self.X` (a bare own-class FIELD read is still not bound); a property READ is also a resolved call to its getter and a WRITE to its setter, and a field that backs an accessor lists those accesses as its own uses. Indexes resolved before resolver 1.3.0-alpha answer 0 for properties until re-resolved (`index --all --resolve-only`). **An ENUM VALUE name works too (2026-09-23, resolver 1.6.0-alpha):** each bound read -- bare (`cmdDelta`) or qualified (`TCommandID.cmdDelta`) -- is a row tagged `[certain, read]` (JSON `mode`); a read the resolver could not bind with certainty (two visible candidates, or a same-named local/const in scope) is NOT listed. **Rows are per SITE, not per caller** (as for properties/fields): one routine reading the value three times yields THREE rows carrying the same `caller_qname`, so a consumer counting callers must dedupe on `caller_qname` (or `caller_qname`+`file`) rather than taking the row count. Indexes resolved before 1.6.0-alpha answer 0 for enum values until re-resolved (`index --all --resolve-only`). **Two known wrong-bind risks, both on BARE enum reads only, neither detectable from the row itself:** (1) a **`with` whose target the index cannot type** -- since resolver 1.8.0-alpha the `with` scope IS modelled: where the target types, a with member wins and the read binds to it, and a bare CALL under an untypable target (a library class, a form descending from `TForm`) binds nothing; only an enum read under such a target keeps its binding, and `enum-read-inside-with` reports the collisions it can prove (measured: 0 such bindings inside with bodies on ORM3 CLIENT -- the 178 counted at 1.6.0 were in `with`-bearing routines, outside the bodies); (2) a **`{$SCOPEDENUMS ON}`** unit -- measured **0** across all 33 project databases, but **NOT measured on the platform LIBRARY indexes**, where RTL/VCL scoped enums live. In either place verify a bare-name binding against the declaration before trusting it. A QUALIFIED read (`TEnum.Value`, `Unit.Value`) is unaffected by both, because the source qualified the name itself. Numbers and method: the enum-value bullet in `CHANGELOG.md`, detail in `docs/MEASURED-enum-value-refs-2026-09-23.md` |
...
| `outline --file F.pas` | all symbols declared in one file. Since extractor 1.17.0 that includes nested-routine locals and inline `var` / `for var` declarations, emitted as `local_var` symbols parented to the INNERMOST routine (`U.T.M.Nested.I`). A forward stub row is tagged `[forward -> line N]` (`forward_target_line` in JSON); both rows are listed. |
...
| `hover --qname U.T.M` | hover card (`--format plain\|md\|json`). On an **interface** it also answers "what satisfies this contract": `Implemented by:` (classes) and `Extended by:` (derived interfaces), capped with a `(+N more)` suffix. Absent entirely when nothing implements it -- never `(none)`. `--format json` also carries `effect_free` (-1/0/1), `effect_summary` (`g,h,s,p<k>,?`) and `effect_witness` -- the stored purity verdict and its first blocker |
...
| `convert-scaffold --from F --to T` | auto-draft a VALID reFind-superset conversion-rules file from the real F/T property trees: concrete `#link` on 1 leaf-name+type match, `???` for ambiguities, `DROPPED` notes for orphaned source props (`--output <f>`, `--surface dfm\|pas` default `dfm` -- restricts auto-linked TARGETS to writable/in-surface leaves) -- see `docs/CONVERSION-RULES.md` |
| `convert-validate --rules F` | parse + validate a reFind-superset conversion-rules DSL; `--from`/`--to` check `#link`/`#default` paths against the real trees; a `#link` glyph expression (`<FromPath> G[I/N]`, stitched `G[1/6]G[2/6]`, per-N alternatives by comma, `G[count]`) is checked in either mode, errors name its column; `line N: warning:` lines never change the exit code; `convert-apply` refuses a G-linked book until CV-2 realises it (`--print-parsed`; exit 0 valid / 1 errors / 2 bad args) |
| `convert-apply --unit F.pas --rules F --db D` | rewrites all 5 conversion surfaces (`.pas` decl retype, `.pas` uses-add, `.dfm` object-block re-emit, `.pas` property/event access-site rewrite, runtime-creator retype + TODO marker) for `.dfm` instances matching a `#convert` rule; dry-run (preview) by default, `--apply` writes for real with `.BCK<n>` backups + `recovery.txt` unless `--no-backup` (`--only Name1,Name2,...` to restrict instances); `--format json` (or `--json`) emits schema `apply/1` -- the six report surfaces plus `items[]`, one typed entry per reported line carrying a machine-readable `kind`, so an agent can DISPATCH on the conversion remainder (the `todos` / `reemit_notes` / `warnings` subset) instead of parsing prose, plus `resolved_defaults[]` -- informational receipts (a property absent because it sat at its declared `default`, carried across explicitly), kept OUT of `items[]` since 2026-09-14 because on a real form they run to thousands and buried the remainder. `items.length` always equals the sum of the six arrays and **`resolved_defaults` is NOT part of that sum** -- **step-by-step agent procedure in [`docs/AI-CONVERT-RUNBOOK.md`](AI-CONVERT-RUNBOOK.md)**; DSL reference in `docs/CONVERSION-RULES.md`. `--castlib <file>` names the `.castlib` whose `enum` blocks translate a `#link` value when the link carries a `: Cast` suffix -- **without it a cast-bearing `#link` passes the value through UNCHANGED**, so pass it whenever the rules file casts. **Unlinked source properties** (present on some converted instance, carried by no `#link`, acknowledged by no `#ignore`) are warned ONCE per (source type, property) as `TFrom.Prop: no #link carries it -- dropped on N of M converted instance(s)` (kind `unlinked-source-property`) -- read the fraction, not the count: `2 of 20` is the two controls somebody deliberately styled and is the STRONGER signal; `20 of 20` is a structural non-mapping. `--format json` carries the same as `unlinked_source_properties` (rows), `unlinked_source_property_sites` (sum of sites) and `unlinked[]` (`from_type`, `path`, `sites`, `instances`); `--no-warn-unlinked` drops the warnings and keeps all three |
| `glyph-vacuum --root D [--root D ...] --output D [--append] [--db X ...]` | measure every streamed graphic under the roots BEFORE writing a glyph rule: `instances.tsv` (one row per component+graphic property: wrapper/format/size/bpp, count property + value + declared default, inferred N, agree, sha), `classes.tsv` (per class: distributions, disagreements, runtime_refs), `skipped.tsv`, `images\`, `gallery.html`; `--append` merges; `--db` only qualifies |
| `cycles` | circular unit deps (`--edges`, `--causes`, `--plan` for a mechanical refactoring playbook: per-symbol recipe, uses decisions, new-unit text, bottom-up edits, checklist with the predicted re-check output) |
| `uses-report --output f.csv` | full uses-graph rollup to CSV (`--depth N`, `--include-external`, `--all-sources`, `--name <pattern>`). A `--name` that matches NO source unit exits 2 with `ERROR: uses-report: no index passed contains a source unit named <pattern>` on stderr and writes NO file -- it does not answer `0 source units, 0 rows` with exit 0 (converter gap 2026-09-16) |
...
| `uses-fix <unit.pas> --project P` | compiler-verified `uses` cleanup (`--remove-unused`, `--apply`). `--format json` emits schema `uses-fix/1`: every CANDIDATE with `kind` (move/remove), `line` and `status` -- `verified` (compiled clean, in the write set), `skipped` (+`reason`), or `deselected` (excluded by `--only`, never compiled). `counts` carries all four totals, so it sums to `items.length`. **`applied` is the OUTCOME, not the flag** -- `--apply` on a unit with nothing to change reports `false` and writes no backup. When a write did happen the document also carries `backup` (the `.bak` path) and `warning` (the per-unit verify is best-effort; run a full build). `--only <unit,...>` restricts which candidates are compiled and written, so a reviewed partial apply is expressible; unmatched names come back in `unmatched` rather than being dropped. The SWEEP form (`uses-fix` with no unit) honours neither flag and says so on stderr |
| `format <file>` | reformat via YADF (`--yadf-path`). **Writes the file in place** -- use `--dry-run` (resolve + version-check, write nothing) or `--diff` (format a copy, print the diff) first. Resolution is `--yadf-path` or `HKCU\Software\YADF\ExePath`; there is no hardcoded fallback. Exit 3 = no YADF, 4 = YADF older than 1.0.6.6 (nothing written), 5 = formatting changed the declared symbols and the file was restored |
...
| `document ... --migrate-pure` | regenerate stored legacy `<para>Pure</para>` lines (purity v1). Without it a block whose ONLY difference is that line stays byte-identical and `doc-drift` does not report it -- the migration is one explicit run |
| project tags (automatic) | on a block reconciled across projects (`dl:shared`, or holding facts this `--db` cannot see) inbound entries read `[ProjA,ProjB]uX.Foo (uX.pas)`: the projects whose index rendered them, named by `--db` base name. A run adds/removes only its own tag, an entry goes when its set empties, untagged legacy entries are kept while this index cannot see their unit, and such lists are never `(+N more)` windows. Convergence is one write per project per line |
| `doc-forget --scope <file\|dir>` | reap tags, text-only: `--project <Tag>` removes it, `--rename <Tag>=<New>` renames it, `--untagged` drops untagged entries in tagged blocks, `--list-tags` counts every tag in scope; dry run unless `--apply` (`--no-backup`) |
...
| `forms-csv --project P --db DB` | test-helper form-navigation CSV, one row per form (`--output <f.csv>`; `--root <TfrmMAIN>` names the form navigation starts from) |
| `import-log <logfile>` | ingest a dcc/msbuild log into the index |
| `export enums\|obsidian` | export enums (firebird-sql/csv/json/delphi-const; `--output <file>` writes it to a file) or an Obsidian vault (`export obsidian --db D --output-dir <dir>`; `--open` launches the vault when done) |
...
| `DRAGLINT_DEBUG` | `1` | Extra internal tracing to stderr. `lint <file>` prints one `[lint-checker] <name>` line per heavy checker it enters (`type-aware`, `flow`, `with-hiding`, `project-rules`, `used-unit-resolvable`, `class-metrics`, `library-store`) -- how to see that a `--rule` run skipped the ones that cannot emit that rule. |
