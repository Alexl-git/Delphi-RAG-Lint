...
- **Task 5 finding (2026-07-29):** Type-alias resolution works correctly; the fixture confirms both direct and aliased generics are recognized as constructors. The YADF reproduction's empty `returns_owner` is due to its exception-handler disposal pattern (correct conservative behaviour, not an alias bug). **Future cycles:** conditional ownership (disposed in exception paths, transferred in normal paths) requires CFG path analysis and schema extension; see `docs/lint/2026-07-29-ownership-yield-finding.md`.
