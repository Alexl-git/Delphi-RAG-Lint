...
and `wiring`, which is RESERVED and never written) -- see 2.15.
...
| `symbol_facts` | 15905 | Per-routine analysis facts (complexity, reads/writes, SQL tables, DFM event, ownership; v19 adds mutated params, UI affinity, external surfaces, and a RESERVED never-written `wiring` column) -- v18+; see 2.15 |
...
| `wiring` | TEXT (nullable) | **v19, RESERVED -- always NULL, by decision (OWN-3, 2026-09-23).** The column exists so the v19 DDL stays stable; the engine never writes it and will not. Do not SELECT it: a consumer that reads it gets NULL on every row of every index. The DI/ORM wiring fact is computed LAZILY at `document`/`hover` time -- never stored -- by joining `di_bindings` / `orm_links` / `fb_relations` / `fb_columns`, exactly as `covered_by` below is computed from the live reverse-call graph. The reason is structural: `orm_links` is written by a SEPARATE post-index pass (`orm-link`), so an index-time value would be empty on every first index and would afterwards reference `symbols.id` values the reindex had already replaced. To get the fact, read the rendered `Registered as:` / `Dataset:` lines from `hover` / the `document` block, or join those four tables yourself (`drag-lint wiring` answers the DI half). |
