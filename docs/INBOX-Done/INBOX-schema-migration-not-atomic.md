> **RETIRED to INBOX-Done/ on 2026-08-16 (session 21).** FIXED 2026-08-16. schema_version was written and COMMITTED inside the first DDL transaction, i.e. BEFORE the additive TryExec ALTERs that follow it, so an upgrade interrupted between the two left a DB advertising the new version while missing the new columns -- and the next open, seeing the version current, had no reason to retry. The stamp now happens LAST, after every idempotent step. Migrate runs on every open, so a partially-applied upgrade now self-heals instead of bricking. Verified: a fresh DB still stamps schema_version 21 and reopens cleanly.
