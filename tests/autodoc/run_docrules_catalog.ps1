...
#   BUILT-IN, project-wide, default_enabled=False -- a routine with a PROVEN
#   effect is called inside Assert, which release builds compile out.
...
#     is called in statement position, so the call cannot do anything.
#   query-name-with-effect       -- a Get*/Is*/Has*/Find*/Can*/Should* routine
#     whose STORED effect summary proves an escaping effect.
#   They ship OFF pending a corpus audit, which as the 2026-09-10 note below
#   says does NOT affect this tally: it counts source=builtin regardless of
#   default state. THIS ASSERTION CAUGHT THE PAIR on the first run after they
#   landed -- README, INSTALL and six wiki pages had already been updated by
#   run_docs_sync_guard, and the BUILT-IN split is pinned only here.
# 2026-09-10: 126 -> 128. PLAN-lint-tree R1 and R2:
#   dfm-property-not-declared        -- a .dfm sets a property that the
#     component's class and its ancestors do not declare.
#   dependent-project-not-recompiled -- a dl:shared unit was changed after
#     another project that also compiles it last built.
# Both are BUILT-IN and project-wide. Both ship default_enabled=False pending
# the R3 audit, which does NOT affect this tally -- it counts source=builtin
# regardless of default state, and conflating the two is how an off-by-default
# rule goes missing from the split without anyone noticing.
# 2026-09-01: 125 -> 126. hardcoded-absolute-path stopped being an external
# .scm and became a BUILT-IN (B7): the query matched any drive-letter literal
# anywhere, so it needed a filesystem sink and a backward walk that a tree-sitter
# pattern cannot express. External count drops by one in the same change.
