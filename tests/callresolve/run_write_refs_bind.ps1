<#
  ...
  the units it uses. A `with` whose target may own the name (D14's scope:
  an untypable target, or a surface that leaves the index), `Result`, a
  function's own name (its result), a routine name, two visible candidates and no candidate at all
  are declines, counted by reason on the `writes:` log line. Since the merge
  with D14 a write INSIDE a with body binds the target's field/property the
  way Delphi does, a name the (fully typed) target lacks falls through to the
  ordinary scopes, and a write AFTER a with statement is no longer declined.

  WHAT THIS GUARD PINS
    check 1  W-*: fifteen writes each bind refs.symbol_id to the named declaration
             -- three of them under or after a `with` (merge of D13 with D14)
    check 2  NEG-*: five writes stay NULL (ambiguous, not found, a `with`
             target that cannot be typed, Result, own name)
...
$pos = @(
  ...
  @{ M = 'W-WITH-MEMBER';    N = 'FCount';    Q = 'uWriteUse.TThing.FCount' },
  @{ M = 'W-WITH-FALLTHROUGH'; N = 'L';       Q = 'uWriteUse.TThing.Run.L' },
  @{ M = 'W-AFTER-WITH';     N = 'L';         Q = 'uWriteUse.TThing.Run.L' })
$neg = @(
  @{ M = 'NEG-AMBIGUOUS'; N = 'GClash' },
  @{ M = 'NEG-NOT-FOUND'; N = 'GUnknown' },
  @{ M = 'NEG-WITH-UNDECIDED'; N = 'GOwn' },
...
try {
  ...
  $want = 'writes: 15 of 20 write ref(s) bound; declined result 1, own-name 1, with-scope 1, ambiguous 1, not-found 1, routine 0, unreadable 0'
