...
$out = cmd /c "call `"$rs`" && cd /d `"$dir`" && dcc64 -B -NSSystem -E`"$dir`" -U`"$repo\src\core;$repo\src\diagnostics;$repo\src\refactor;$repo\src\parser;$repo\src\analysis;$repo\src\preprocess;$repo\third_party\delphi-tree-sitter`" `"$dir\ExtractMethodTests.dpr`"" 2>&1
