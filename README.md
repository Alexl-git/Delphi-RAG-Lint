...
| `lint-tree --unit <B.pas> --db <db>` | **Does an interface edit to `B.pas` reach any dependent?** Fingerprints B's interface and compares it with an edit-episode baseline (`--write-baseline` to capture, `--baseline` to diff). `--buffer <f>` fingerprints an UNSAVED buffer. Exits 0 whether or not it found anything -- exit 2 means it could not run. Answers the question `lint-all` cannot: removing an interface symbol that dependents still use produces ZERO `lint-all` findings (measured 2026-09-10). |
