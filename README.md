...
| [`query find-callers`](https://github.com/Alexl-git/Delphi-RAG-Lint/wiki/query-find-callers) `--name <n>` | Every call-site for a symbol, with source context | `--context N`, `--resolved` (precise call-edges; since 2026-09-16 a PROPERTY or FIELD name lists its bound reads/writes too, tagged `[certain, read]` / `[certain, write]`) |
...
external `.scm`). As of v1.14.0-alpha: **181 rules across 16 categories -- 128
...
**v1.14.0-alpha**; development continues daily.
