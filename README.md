...
| [`query find-callers`](https://github.com/Alexl-git/Delphi-RAG-Lint/wiki/query-find-callers) `--name <n>` | Every call-site for a symbol, with source context | `--context N`, `--resolved` (precise call-edges; every row names the CALL SITE -- text `(<file>:<line>)`, JSON `line` -- with the caller's declaration line under JSON `caller_line`; since 2026-09-16 a PROPERTY or FIELD name lists its bound reads/writes too, tagged `[certain, read]` / `[certain, write]`; since 2026-09-23 an ENUM VALUE name lists every bound read the same way, tagged `[certain, read]`. **Two known wrong-bind risks, both on BARE reads only:** a routine containing a `with` block -- `with` scope is not modelled, and 178 bound bare reads sit inside one on ORM3 CLIENT alone -- and a `{$SCOPEDENUMS ON}` unit, which measured 0 across the whole project corpus but was NOT measured on the platform LIBRARY indexes, where RTL/VCL scoped enums live. In either place, verify a bare-name binding against the declaration before trusting it; qualified reads (`TEnum.Value`) are unaffected. Numbers and method: the enum-value bullet in `CHANGELOG.md`) |
...
| `query descendants` `--of <a>` | The reverse of `ancestors`: every **class** descending from `<a>`, across all scanned DBs. Derived interfaces are excluded by design (this backs the conversion editor's class pickers) | `--json` (exit 1 = no such ancestor) |
...
| `lint-all --db <db>` | Full project report. Since the per-file verb gained the store-backed project rules, what `lint-all` *uniquely* adds is narrower than it used to be: cross-file duplicate code, `interface-reference-cycle`, exception-class enrichment, and doc-drift/missing-doc across the whole project (`review-marker-unused` and `review-marker-malformed` moved to the per-file verb too, 2026-09-03) | `--project <.dproj>` (report only that project's compile closure), `--rule <id>` (report only that rule; an OFF-by-default rule named here is opted in for the run, as with `lint`), `--output <file>`, `--json`, `--quiet` (no per-file progress lines on stderr), `--lint-third-party`, `--no-preprocess` (lint the raw bytes, including branches the compiler never sees -- the default now resolves conditionals the same way the indexer does) |
...
| `document ... --migrate-pure` | Regenerate stored legacy `<para>Pure</para>` facts (purity v1). **Without it** a block whose ONLY difference from a fresh render is that line is left byte-identical and `doc-drift` agrees -- the purity migration is one deliberate run, not churn on every `document` | works on `--qname` / `--unit` / `--project` / `document-all` |
| `document` on a shared unit -- **project tags** | On a block reconciled across projects (a `dl:shared` unit, or one holding facts this index cannot see) inbound entries carry the projects that rendered them: `Called from: [DataCopy,DataCopyTests]uX.Foo (uX.pas)`. The tag is the `--db` base name (none with several `--db`). A run adds or removes only ITS tag; an entry goes when its set empties; untagged legacy entries keep the old rules (kept while this index cannot see their unit). Lists on such a block are never windowed `(+N more)`. A private unit never gets a tag | automatic |
| `doc-forget --scope <file\|dir>` | Reap project tags from inbound doc facts, text-only (no index needed -- for a project that was retired or renamed) | `--project <Tag>` (remove), `--rename <Tag>=<New>`, `--untagged` (drop untagged entries in tagged blocks), `--list-tags`, `--apply`, `--no-backup` |
...
| `check-ast <file>` | Run tree-sitter lint rules without compiling | `--rule <id>`, `--format text\|json` |
...
| `enum-read-inside-with` | warning | A bare name inside a `with` body names both an enum value and a with-target member -- the compiler binds the member, the index records the enum value |
