...
type
  TBrowseHandler = class(TComponent)
    ...
    public
      constructor Create(AOwner: TComponent; AEdit: TEdit; AOpenDlg: TOpenDialog); reintroduce;
      procedure DoBrowse(Sender: TObject);
  end;

constructor TBrowseHandler.Create(AOwner: TComponent; AEdit: TEdit; AOpenDlg: TOpenDialog);
begin
...
begin
  if FOpenDlg.Execute then FEdit.Text:= FOpenDlg.FileName;
end;
...
var
  ...
  cbAutoReindex  : TCheckBox        ;
  cbScanLibraries: TCheckBox        ;
  cbHover        : TCheckBox        ;
  cbCompletion   : TCheckBox        ;
  cbSignature    : TCheckBox        ;
  cbDiag         : TCheckBox        ;
  cbInlineMarkers: TCheckBox        ;
  cbErrInline    : TCheckBox        ;
  cbWarnInline   : TCheckBox        ;
  cbHintInline   : TCheckBox        ;
  cbInfoInline   : TCheckBox        ;
  ...
  function AddCheck(AY: Integer; const ACaption: string; AChecked: Boolean): TCheckBox;
  begin
...
begin
  ...
  try
    ...
    Inc(Y, 8);
    cbAutoIndex    := AddCheck(Y, 'Auto-index project when .dproj opens'              , Settings.AutoIndex        ); Inc(Y, 24);
    cbAutoReindex  := AddCheck(Y, 'Auto-reindex on file save (.pas, .dpr, .dfm)'      , Settings.AutoReindexOnSave); Inc(Y, 24);
    cbScanLibraries:= AddCheck(Y, 'Scan libraries (RTL + DevExpress + browsing paths)', Settings.ScanLibraries    ); Inc(Y, 24);
    cbHover        := AddCheck(Y, 'Enable Hover at Cursor'                            , Settings.EnableHover      ); Inc(Y, 24);
    cbCompletion   := AddCheck(Y, 'Enable Show Completion'                            , Settings.EnableCompletion ); Inc(Y, 24);
    cbSignature    := AddCheck(Y, 'Enable Show Signature Help'                        , Settings.EnableSignature  ); Inc(Y, 24);
    cbDiag         := AddCheck(Y, 'Enable Run Diagnostics'                            , Settings.EnableDiagnostics); Inc(Y, 24);

    Inc(Y, 8);
    cbInlineMarkers:= AddCheck(Y, 'Enable inline markers (gutter + underline)', Settings.EnableInlineMarkers); Inc(Y, 24);
    cbErrInline    := AddCheck(Y, '  Show errors inline'                      , Settings.ShowErrorsInline   ); Inc(Y, 24);
    cbWarnInline   := AddCheck(Y, '  Show warnings inline'                    , Settings.ShowWarningsInline ); Inc(Y, 24);
    cbHintInline   := AddCheck(Y, '  Show hints inline'                       , Settings.ShowHintsInline    ); Inc(Y, 24);
    cbInfoInline   := AddCheck(Y, '  Show info inline'                        , Settings.ShowInfoInline     ); Inc(Y, 24);

    Inc(Y, 8);
    cbCodeLens:= AddCheck(Y, 'Enable inline code lens ([N callers])', Settings.EnableCodeLens); Inc(Y, 24);

    Inc(Y, 8);
    cbWorkspace:= AddCheck(Y, 'Enable workspace mode (auto-detect .drag-lint-workspace.json)', Settings.EnableWorkspaceMode); Inc(Y, 24);

    cbHoverTooltip:= AddCheck(Y, 'Enable hover tooltip (caret-based, 600ms dwell)', Settings.EnableHoverTooltip); Inc(Y, 24);
