...
var
  ...
  function NewLabel(AParent: TWinControl; const ACap: string; AX, AY: Integer): TLabel;
  begin
  ...
  function NewEdit(AParent: TWinControl; AX, AY, AW: Integer; const ATxt: string): TEdit;
  begin
  ...
  function NewCheck(AParent: TWinControl; AX, AY: Integer; const ACap: string; AChecked: Boolean): TCheckBox;
  begin
...
begin
  ...
  GY:= GH - 4;
  FCbAutoIndex:= NewCheck(FGrpAutoIndex, LM, GY, 'Auto-index project when .dproj opens', False);
  Inc(GY, CH);
  FCbAutoReindex:= NewCheck(FGrpAutoIndex, LM, GY, 'Auto-reindex on file save (.pas, .dpr, .dfm)', False);
  Inc(GY, CH);
  FCbScanLibraries:= NewCheck(FGrpAutoIndex, LM, GY, 'Scan libraries (RTL + DevExpress + browsing paths) on index', False);
  ...
  GY:= GH - 4;
  FCbCodeLens:= NewCheck(FGrpCodeLens, LM, GY, 'Enable inline code lens ([N callers] next to method declarations)', False);

  { --- Workspace --- }
  FGrpWorkspace:= NewGroup('Workspace (v0.34)', GH + CH + 4);
  GY:= GH - 4;
  FCbWorkspace:= NewCheck(FGrpWorkspace, LM, GY, 'Enable workspace mode (auto-detect .drag-lint-workspace.json)', False);

  { --- Hover Tooltip --- }
  FGrpHoverTip:= NewGroup('Hover Tooltip (v0.35)', GH + CH + 4);
  GY:= GH - 4;
  FCbHoverTooltip:= NewCheck(FGrpHoverTip, LM, GY, 'Enable hover tooltip (caret-based, 600ms dwell)', False);
end; // begin
