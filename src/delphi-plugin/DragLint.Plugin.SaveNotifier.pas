...
type
  TDragLintSaveNotifier = class(TInterfacedObject, IOTANotifier, IOTAModuleNotifier)
    private
      FModule       : IOTAModule;
...
begin
  Result:= False;
  if GLastReindexTick = nil then GLastReindexTick:= TDictionary<string, Cardinal>.Create;
  Key:= LowerCase(AFile);
  Now:= GetTickCount;
  if GLastReindexTick.TryGetValue(Key, Last) and (Now - Last < REINDEX_DEBOUNCE_MS) then Exit(True);
...
class procedure TDragLintSaveNotifier.SpawnIndexerFile( const AExePath, AFilePath, ADbPath: string);
var
  CmdLine   : string                    ;
  SI        : TStartupInfoW             ;
...
begin
  ...
  CmdLine:= Format('"%s" index "%s" --db "%s" --deep', [AExePath, AFilePath, ADbPath]);
...
begin
  ...
  try
    ...
    TThread.Queue(
      nil,
      procedure var Svc: IOTAMessageServices; begin try if Supports(BorlandIDEServices, IOTAMessageServices, Svc) then Svc.AddTitleMessage( Format('drag-lint: reindexing %s...',
...
begin
  ...
  if GRegistrations = nil then GRegistrations:= TList<TSavedRegistration>.Create;
  Reg.Module:= AModule;
...
if GRegistrations   <> nil then FreeAndNil(GRegistrations  );
if GRegLock         <> nil then FreeAndNil(GRegLock        );
if GLastReindexTick <> nil then FreeAndNil(GLastReindexTick);
