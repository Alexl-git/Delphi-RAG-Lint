...
const
  { Exact leaf names (case-insensitive) that are always treated as excluded. }
  PRUNE_EXACT: array[0..6] of string = (
    '__history', '__recovery', '.git', '.svn', '.hg', 'node_modules', '.vs'
  );

  { Glob patterns (applied via TGlob.Matches on the leaf name). }
  PRUNE_GLOB: array[0..0] of string = (
    '*backup*'
  );
...
begin
  ...
  for I := Low(PRUNE_EXACT) to High(PRUNE_EXACT) do
    if Lo = PRUNE_EXACT[I] then
      Exit(True);
  for I := Low(PRUNE_GLOB) to High(PRUNE_GLOB) do
    if TGlob.Matches(ALeaf, PRUNE_GLOB[I]) then
      Exit(True);
  Result := False;
end;
...
begin
  ...
  if not TDirectory.Exists(NormRoot) then
    Exit;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            begin
              if MatchedNames.IndexOf(PS.Name) < 0 then
                MatchedNames.Add(PS.Name);
              Break;
            end;
          end;
        end;
