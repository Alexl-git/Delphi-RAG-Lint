...
function TIndexManifest.FindSection(const AName: string;
  out ASection: TIndexSection): Boolean;
var
  I: Integer;
begin
...
begin
  ...
  try
    ...
    begin
      ...
      if (V <> nil) and (V.Value <> '') then
        Result.OutDir := V.Value;

      V := JIndexes.GetValue('exclude');
      if V is TJSONArray then
        Result.GlobalExclude := JsonStrArr(TJSONArray(V));
      ...
      begin
        ...
        for I := 0 to JSections.Count - 1 do
          if JSections.Items[I] is TJSONObject then
            Result.Sections[I] := ParseSection(TJSONObject(JSections.Items[I]));
      end;
    end;
  finally
    Root.Free;
...
var
  ...
  begin
    ...
    begin
      ...
      if MatchIdx >= 0 then
        ADest.Sections[MatchIdx] := ASrc.Sections[K]
      else
      begin
...
begin
  ...
  else if HaveLocal then
    Result := LocalManifest
  else if HaveGlobal then
    Result := GlobalManifest
  else
  begin
