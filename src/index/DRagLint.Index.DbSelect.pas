...
type
  ...
  public
    ...
    class function Resolve(const AManifest: TIndexManifest;
      const APlatform: string; AResolver: TProjectResolver;
      ARequireExists: Boolean = True): TArray<string>; static;
  end;

implementation

class function TDbSelect.Resolve(const AManifest: TIndexManifest;
  const APlatform: string; AResolver: TProjectResolver;
  ARequireExists: Boolean): TArray<string>;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if (LibDbPath = '') and SameText(PS.Platform, APlatform) then
        LibDbPath := PS.DbPath;
    end;
  end;
