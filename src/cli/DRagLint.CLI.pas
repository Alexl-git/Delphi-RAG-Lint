  ...
  try
    ...
    begin
      ...
      if Sccs.Count = 0 then
        Writeln('No circular unit dependencies. Nothing to do.');
      ...
      begin
        ...
        if not HasIntf then
        begin
          Writeln('Status: **implementation-only** (legal in Delphi, low impact). ' +
            'Skip unless it hurts build times.');
          Writeln('');
          Continue;
        end;
        ...
        begin
          ...
          for var B in Comp do
            ...
            begin
              ...
              if (LayerOf(pa) = 'COMMON') and
                 ((LayerOf(pb) = 'CLIENT') or (LayerOf(pb) = 'SERVER')) then
          ...
          begin
            ...
            try
              ...
              begin
                ...
                for var Sym in Store.FindSymbolsByExactName(R.NameText) do
                  ...
                  begin
                    ...
                    Writeln(Format('- `%s` interface uses `%s` (%s) at `%s:%d`; ' +
                      'declared in `%s:%d`.',
                      [A, R.NameText, Sym.Kind.ToText, ExtractFileName(Apath),
                       R.StartLine, ExtractFileName(Bp), Sym.StartLine]));
              ...
              if not AnyFound then
                Writeln(Format('- `%s` interface uses `%s` but the index could not ' +
                  'resolve the symbol (likely a `set` type). **Open `%s` and find ' +
                  'what its interface uses from `%s` by hand.**',
                  [A, B, ExtractFileName(Apath), B]));
        ...
        if InvFrom <> '' then
        begin
          Writeln(Format('**Invert the dependency.** `%s` (COMMON) must not depend ' +
            'on `%s` (CLIENT/SERVER) -- that inversion is what closes the cycle.',
            [InvFrom, InvTo]));
          Writeln('');
          Writeln('Steps:');
          Writeln(Format('1. Find what `%s` uses from `%s` (the `%s uses %s ' +
            '[implementation]` edge).', [InvFrom, InvTo, InvFrom, InvTo]));
          Writeln(Format('2. Declare an **interface** in `%s` (or a new COMMON leaf ' +
            'unit) for that behaviour.', [InvFrom]));
          Writeln(Format('3. Make `%s` IMPLEMENT that interface; have `%s` use the ' +
            'interface, not `%s`.', [InvTo, InvFrom, InvTo]));
          Writeln(Format('4. Remove `%s` from `%s`''s uses clause.', [InvTo, InvFrom]));
        end
        else
        begin
          Writeln('**Extract the shared contract** into a new leaf unit both can ' +
            'depend on (it must use NEITHER unit in this cycle).');
          Writeln('');
          Writeln('Steps:');
          Writeln('1. For each symbol under "Why it cycles", create/reuse a leaf ' +
            'unit (e.g. `<Unit>.Contracts.pas`) holding ONLY that declaration.');
          Writeln('2. Move the declaration there; add the new unit to the declaring ' +
            'unit''s uses.');
          Writeln('3. In each consumer, replace the cycle-partner in the ' +
            '**interface** uses with the new contracts unit.');
          Writeln('4. Register the new unit in the .dpr/.dproj.');
        end;
        Writeln('5. Build CLIENT + SERVER.');
        Writeln(Format('6. **Verify:** `drag-lint cycles --db <db>` -- cycle %d ' +
          'should be gone.', [I + 1]));
        Writeln('');
      end;
    end
