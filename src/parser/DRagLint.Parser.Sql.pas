...
type
  TSqlState = class
    ...
    function  AddSymbol(AKind: TSymbolKind; const AName, AQName: string;
      AParentIdx: Integer; AStartLine, AStartCol, AEndLine, AEndCol: Integer;
      const ASignature: string = ''): Integer;
    procedure AddRef(const AKind, ANameText: string;
...
function TSqlState.AddSymbol(AKind: TSymbolKind; const AName, AQName: string;
  AParentIdx, AStartLine, AStartCol, AEndLine, AEndCol: Integer;
  const ASignature: string): Integer;
var
  S: TSymbol;
begin
...
procedure TSqlState.AddRef(const AKind, ANameText: string;
  AStartLine, AStartCol, AEndLine, AEndCol: Integer);
var
  R: TReference;
begin
...
procedure ComputeLineCol(const AText: string; APos: Integer;
...
var
  ...
  C, C2: Char;
  Buf: TStringBuilder;
  InLineCmt, InBlockCmt, InString: Boolean;
begin
  N := Length(AText);
...
procedure ParseColumnList(const AText: string; ATableIdx: Integer;
  const ATableName: string; AStartPos, AEndPos: Integer; AState: TSqlState);
...
var
  ...
  begin
    ...
    J := 1;
    while (J <= Length(Trimmed)) and (CharInSet(Trimmed[J], ['A'..'Z','a'..'z','0'..'9','_','$'])) do
      Inc(J);
    Name := Trim(Copy(Trimmed, 1, J - 1));
    TypeText := Trim(Copy(Trimmed, J + 1, MaxInt));
    if Name = '' then Exit;
    ComputeLineCol(AText, AItemStart, ColLine, ColCol);
    AState.AddSymbol(skSqlColumn, Name, ATableName + '.' + Name,
      ATableIdx, ColLine, ColCol, ColLine, ColCol + Length(Name),
      TypeText);
  end;
...
function TFirebirdSqlParser.Parse(const ASource: TBytes;
  const AFilePath: string): TParseResult;
var
  State:   TSqlState;
  Raw:     string;
  Cleaned: string;
  RxTable, RxGen, RxTrig, RxView, RxProc, RxExc, RxDom, RxIdx: TRegEx;
  M:       TMatch;
  Line, Col, EndLine, EndCol: Integer;
  Name:    string;
  Idx:     Integer;
  OpenP, CloseP: Integer;
begin
  ...
  try
    RxTable := TRegEx.Create(
      '\bCREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([A-Za-z_$][A-Za-z0-9_$]*)\s*\(',
      [roIgnoreCase]);
    RxGen := TRegEx.Create(
      '\bCREATE\s+(?:GENERATOR|SEQUENCE)\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    RxTrig := TRegEx.Create(
      '\bCREATE\s+(?:OR\s+ALTER\s+)?TRIGGER\s+([A-Za-z_$][A-Za-z0-9_$]*)\s+(?:FOR|ON)\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    RxView := TRegEx.Create(
      '\bCREATE\s+(?:OR\s+ALTER\s+)?VIEW\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    RxProc := TRegEx.Create(
      '\bCREATE\s+(?:OR\s+ALTER\s+)?PROCEDURE\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    RxExc := TRegEx.Create(
      '\bCREATE\s+EXCEPTION\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    RxDom := TRegEx.Create(
      '\bCREATE\s+DOMAIN\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    RxIdx := TRegEx.Create(
      '\bCREATE\s+(?:UNIQUE\s+)?(?:ASC|DESC|ASCENDING|DESCENDING)?\s*INDEX\s+([A-Za-z_$][A-Za-z0-9_$]*)\s+ON\s+([A-Za-z_$][A-Za-z0-9_$]*)',
      [roIgnoreCase]);
    ...
    begin
      ...
      if CloseP > OpenP then
        ComputeLineCol(Raw, CloseP, EndLine, EndCol)
      else
      begin
        EndLine := Line;
        EndCol  := Col + Length(Name);
      end;
      Idx := State.AddSymbol(skSqlTable, Name, Name, -1,
        Line, Col, EndLine, EndCol);
      if CloseP > OpenP then
        ParseColumnList(Raw, Idx, Name, OpenP + 1, CloseP - 1, State);
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      State.AddSymbol(skSqlGenerator, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      Idx := State.AddSymbol(skSqlTrigger, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      State.AddRef('sql_table_ref', M.Groups[2].Value,
        Line, Col, Line, Col + M.Length);
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      State.AddSymbol(skSqlView, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      State.AddSymbol(skSqlProcedure, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      State.AddSymbol(skSqlException, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      State.AddSymbol(skSqlDomain, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      M := M.NextMatch;
    end;
    ...
    begin
      Name := M.Groups[1].Value;
      ComputeLineCol(Raw, M.Index, Line, Col);
      State.AddSymbol(skSqlIndex, Name, Name, -1,
        Line, Col, Line, Col + Length(Name));
      State.AddRef('sql_table_ref', M.Groups[2].Value,
        Line, Col, Line, Col + M.Length);
      M := M.NextMatch;
