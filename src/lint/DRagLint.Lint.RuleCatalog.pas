...
begin
  ...
  try
    ...
    B('global-only-uses-edge', 'project-wide', 'info',    'A global variable is the ONLY link from one unit to another -- relocating it (or injecting it, when it is interface-typed) deletes the uses edge', False); { OFF by default -- a design opinion about coupling, not a defect, and it is a full refs scan (0.92 s on a 144 MB index), so it is gated on OptedIn in TProjectLintRules.Run rather than merely filtered afterwards. Opt in via "enabled" }
