...
begin
  ...
  try
    ...
    B('uses-global-census', 'project-wide', 'info', 'How many of a used unit''s globals and consts this unit actually draws -- acknowledge a travels-together pair with // dl:census-ok <unit>', True); { ON by default since 2026-08-30 on the owner's ruling (see global-only-uses-edge above). A census rather than a defect report, but a small one -- and `// dl:census-ok <unit>` is the acknowledgement path for a pair that legitimately travels together. Still gated on OptedIn in TProjectLintRules.Run for its cost (1.26 s full refs scan); the gate now defaults to KEEPING it. }
