...
begin
  ...
  try
    ...
    B('duplicate-global-decl', 'project-wide', 'warning', 'The same const or var name is declared at interface unit level in two or more units -- which one compiles depends on uses order'); { ON by default and NOT OptedIn-gated, unlike its sibling above: symbols-only, no refs join, measured 0.05 s on a 144 MB index, 11 findings on ORM3 and 0 on this repo. Owner's own named mistake class }
