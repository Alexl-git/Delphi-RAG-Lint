...
type
  ...
  TFbSnapshot = class
  public
    class function Run(const AConnectionString: string;
      const ASqliteStore: TSQLiteSymbolStore): TFbSnapshotStats; static;
  end;
...
begin
  ...
  try
    Q.Connection := AConn;
    Q.SQL.Text :=
      'SELECT FIRST 1 1 FROM RDB$RELATIONS WHERE UPPER(RDB$RELATION_NAME) = :n';
...
begin
  ...
  try
    ...
    Q.SQL.Text :=
      'UPDATE fb_relations ' +
      'SET sql_table_symbol_id = (' +
      '  SELECT s.id FROM symbols s ' +
      '  WHERE s.kind = ''sql_table'' ' +
      '    AND UPPER(s.name) = UPPER(fb_relations.name) ' +
      '  LIMIT 1)';
    Q.ExecSQL;
    Q.SQL.Text :=
      'UPDATE fb_columns ' +
      'SET sql_column_symbol_id = (' +
      '  SELECT c.id FROM symbols c ' +
      '  JOIN   symbols t ON t.id = c.parent_id ' +
      '  WHERE c.kind = ''sql_column'' AND t.kind = ''sql_table'' ' +
      '    AND UPPER(c.name) = UPPER(fb_columns.name) ' +
      '    AND UPPER(t.name) = (' +
      '      SELECT UPPER(name) FROM fb_relations ' +
      '      WHERE id = fb_columns.relation_id) ' +
      '  LIMIT 1)';
    Q.ExecSQL;
  finally
    Q.Free;
...
class function TFbSnapshot.Run(const AConnectionString: string;
  const ASqliteStore: TSQLiteSymbolStore): TFbSnapshotStats;
var
  ...
  begin
    ...
    try
      ...
      begin
        P := Pos('=', Pair);
        if P > 0 then
          Params.Add(Trim(Copy(Pair, 1, P-1)) + '=' + Trim(Copy(Pair, P+1, MaxInt)));
      end;
      if Params.IndexOfName('DriverID') = -1 then
    ...
    try
      ...
      try
        ...
        QSrc.SQL.Text :=
          ...
          '       COALESCE(RDB$SYSTEM_FLAG, 0) AS SYSTEM_FLAG, ' +
        ...
        QIns.SQL.Text :=
          'INSERT INTO fb_relations(name, owner, system_flag, description, snapshot_at) ' +
          'VALUES (:n, :o, :sf, :d, :sa) ';
      ...
      try
        ...
        QSrc.SQL.Text :=
          ...
          '       TRIM(RF.RDB$FIELD_NAME)    AS NM, ' +
          '       RF.RDB$FIELD_POSITION       AS POS, ' +
          '       TRIM(RF.RDB$FIELD_SOURCE)   AS SRC, ' +
          '       F.RDB$FIELD_TYPE            AS FT, ' +
          '       F.RDB$FIELD_LENGTH          AS FL, ' +
          '       F.RDB$FIELD_SCALE           AS FSC, ' +
          '       F.RDB$FIELD_PRECISION       AS FPR, ' +
          '       COALESCE(RF.RDB$NULL_FLAG, 0) AS NULLFL, ' +
          '       F.RDB$DEFAULT_SOURCE        AS DEFV, ' +
        ...
        QIns.Connection := Sqlite;
        QIns.SQL.Text :=
          'INSERT INTO fb_columns(' +
          '  relation_id, name, position, field_source, field_type, ' +
          '  field_length, field_scale, field_precision, nullable, ' +
          '  default_value, description, snapshot_at) ' +
          'VALUES (:rid, :n, :p, :src, :ft, :fl, :fsc, :fpr, :nul, :def, :d, :sa)';
      ...
      begin
        ...
        try
          QSrc.Connection := FbConn;
          QSrc.SQL.Text :=
            'SELECT DS_ID, DESCRIPTION, SELECT_SQL, UPDATE_SQL, INSERT_SQL, ' +
            '       DELETE_SQL, REFRESH_SQL, NAME_GENERATOR, KEY_FIELD, ' +
            '       UPDATE_TABLE_NAME, UPDATE_ONLY_MODIFIED_FIELDS, ' +
            '       CONDITIONS, FIB$VERSION ' +
            'FROM FIB$DATASETS_INFO';
          QSrc.Open;
          QIns.Connection := Sqlite;
          QIns.SQL.Text :=
            'INSERT INTO fb_datasets(ds_id, description, select_sql, ' +
            '  update_sql, insert_sql, delete_sql, refresh_sql, ' +
            '  name_generator, key_field, update_table_name, ' +
